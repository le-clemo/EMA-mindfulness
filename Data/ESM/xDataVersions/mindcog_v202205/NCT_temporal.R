NCT_temporal <- function(data1, data2, type="temporal",
                gamma, it = 100, binary.data=FALSE, 
                paired=TRUE, weighted=TRUE, AND=TRUE, abs=TRUE,
  ...
  nw1 <- estimateNetwork(x1, vars = nodeVars,
  ...
  # if (is.list(nw2)) nw2 <- nw2[["graph"]][[type]]
  
  nw2 <- estimateNetwork(x1, vars = nodeVars,
                         default = "graphicalVAR",
                         idvar = "subjB",
                         dayvar = "blockAssessmentDay",
                         beepvar = "dayBeepNum")
  ...
  for (i in 1:it)
  {
    ...
    if(paired==FALSE)
    {
      ...
      # if (is.list(r1perm)) r1perm <- r1perm[["graph"]][[type]]
      # 
      # r2perm <- do.call(estimator,c(list(x2perm),estimatorArgs))
      # if (is.list(r2perm)) r2perm <- r2perm[["graph"]][[type]]
      
      r1perm <- estimateNetwork(x1perm, vars = nodeVars,
                                default = "graphicalVAR",
                                idvar = "subjB",
                                dayvar = "blockAssessmentDay",
                                beepvar = "dayBeepNum")
      
      r1perm <- r1perm[["graph"]][[type]]      
      
      
      r2perm <- estimateNetwork(x2perm, vars = nodeVars,
                             default = "graphicalVAR",
                             idvar = "subjB",
                             dayvar = "blockAssessmentDay",
                             beepvar = "dayBeepNum")
      
      r2perm <- r2perm[["graph"]][[type]]
    ...
    print(diffedges.perm[i,])
    print(abs(r1perm-r2perm)[upper.tri(abs(r1perm-r2perm))])

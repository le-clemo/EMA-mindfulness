...
hist(data$meanRT, breaks = 20)
qqnorm(data$meanRT)
qqline(data$meanRT)

hist(data$propCor, breaks = 20)
qqnorm(data$propCor)
qqline(data$propCor)
...
avg2 <- ddply(data, c("group", "intervention", "phaseBeepNum", "meanRT"), summarise,
              N    = length(meanRT),
              sd   = sd(meanRT),
              se   = sd / sqrt(N))
...
sticky.max <- bam(stickiness_gam ~ s(phaseBeepNum) + group * intervention * thoughtsValence * thoughtsTime +

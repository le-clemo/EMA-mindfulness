...
#read in data
data <- read.csv(paste(file_path, 'mindcog_db_2022-09-27.csv', sep = "/"), sep = ";") 


################################# load Medoq info and clean up #################################################
#get sheet names
#sheetnames <- excel_sheets('Medoq_informatie_2.xlsx')
mylist <- lapply(excel_sheets(paste(file_path, 'Medoq_informatie_2.xlsx', sep = "/")), read_excel, path = paste(file_path, 'Medoq_informatie_2.xlsx', sep = "/"))
...
write.csv(na_data, file = paste(file_path, "patientID_issues.csv", sep = "/"))
...
write.csv(matchingData, file = paste(file_path, "matchingData.csv", sep = "/"))
...
#create means (there are 4 NA and 4 PA variables)
data$meanNA <- data$sumNA / 4
data$meanPA <- data$sumPA / 3
...
################################### Sleep Measures ####################################
# Code von Clara Schier to compute sleep duration
...
# add sleep quality of first measurement per day to all measurements of that day
...
data <- data %>% select(patient_id, id, subject, group, intervention, phase, block, respondent_id,
                        ...
                        restednessWakeup, columnNames[36:61], sumPA, sumNA, meanNA, meanPA,
...
data$thoughtsTime <- factor(data$thoughtsTime)
levels(data$thoughtsTime) <- c("past", "present", "future")

data$thoughtsValence <- factor(data$thoughtsValence)
levels(data$thoughtsValence) <- c("negative", "neutral", "positive")

data$aloneCompany <- factor(data$aloneCompany)
levels(data$aloneCompany) <- c("alone", "in company")
...
#save data to csv
write.csv(data, paste(file_path, "preprocessed_data.csv", sep = "/"), row.names = FALSE)

...
load_existing_file <- function(addToFile, permuteBy, ){
...
NPT <- function(data, nodes, addToFile, permuteBy="nodes", iterations=100, idvar="subjB", dayvar="assessmentDay", beepvar="dayBeepNum",
  ...
  if(hasArg(addToFile)){
    ...
    gs.cont.vec[1:(prev_iter)] <- testStats$GlobalStrength$Contemporaneous
    gs.temp.vec[1:(prev_iter)] <- testStats$GlobalStrength$Temporal
  ...
  if(permuteBy == "nodes"){
   ...
   # print("testSummary filled --> done")
    ...
    testStats$EdgeWeights <- list(Temporal = edgeWeights$Temporal, Contemporaneous = edgeWeights$Contemporaneous)
    testStats$GlobalStrength <- list(Temporal = gs.temp.vec, Contemporaneous = gs.cont.vec)
    
    print("Test output")
    print(edgeWeights$Temporal)
    print(gs.cont.vec)
    print("Done")
    ...
    # Centrality = list(), Temporal = list(EdgeWeights = list(), GlobalStrength = list()),
   ...
   # print("++++++++++++++++++++++++ Initialization done ++++++++++++++++++++++++")
    ...
    for(n in nodeVars){
      ...
      print(testStats[[permuteSet[2]]]$Centrality[[n]][1,])
      print(testStats[[permuteSet[1]]]$Centrality[[n]][1,])
      
      testStats$Difference[[n]][1,] <-  testStats[[permuteSet[2]]]$Centrality[[n]][1,] - testStats[[permuteSet[1]]]$Centrality[[n]][1,]
      
    }
    
    print("Cent test stat calculated")
  ...
  #  print("testSummary filled --> done")
    ...
    testStats[[g]]$EdgeWeights <- list(Temporal = edgeWeights$Temporal, Contemporaneous = edgeWeights$Contemporaneous)
    testStats[[g]]$GlobalStrength <- list(Temporal = gs.temp.vec, Contemporaneous = gs.cont.vec)
...
test_1 <- NPT(data_t[which((data_t$group == "controls") & (data_t$phase == "pre")),], nodes = nodeVars,
                       iterations = 2)

test_2 <- NPT(data_t[which((data_t$phase == "pre")),], nodes = nodeVars,
            iterations = 2, permuteBy = "group")
